...
@Suite("Stacked Cancelation (Flat)", .tags(.cancelation, .mapping), .serialized)
struct StackedCancellationTests {
    ...
    func cancelationByChildError() async throws {
        ...
        let stream = await (1...upperBound).stream.map { _ in
            heavyJob()
            counter.add(1, ordering: .sequentiallyConsistent)
        }.flatMap { Void -> Array<Void> in throw TestError.example }
    ...
    func useAfterCancel() async {
        let stream = await (1...upperBound).stream.map { $0 }.flatMap { [$0] }
        stream.cancel()
